...
public class ArquivoConf {
    ...
    static Iterator<Arquivo> itr;
    

    public static void carregar(String p_arquivoConf) throws IOException {
        ...
        for (int i = 0; i < v_retorno.size(); i++) {
            try {
                Arquivos.add(new Arquivo(new File(path + v_retorno.get(i).trim()), v_retorno.get(i), path));
            } 
            catch (IOException e) {
                e.printStackTrace();
            }
